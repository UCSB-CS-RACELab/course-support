...
def handler(event, context): #can be called directly or triggered by AWS
    ...
    if not context: #calling from main so set the profile we want
        if profile is None: #cannot continue if profile is not set!
            print("ERROR:handler: AWS IAM profile (credentials) not set")
            sys.exit(-1)
    try: #setup a connection to AWS services with default profile
...
def main():
    parser = argparse.ArgumentParser(description='LamRekog tool: passes an AWS S3 image file (bucket_name/key) to AWS Rekognition and prints out the list of labels that the service returns (or None).')
